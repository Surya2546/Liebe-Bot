    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "miley_fbanned_users.json"
                update.effective_message.reply_document(document=output, filename="miley_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "miley_fbanned_users.csv"
...
For help and updates visit @Liebe__News and Join @Liebe__Support aslo 🙂
Command:
